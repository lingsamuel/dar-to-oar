...
namespace DARtoOAR {
    internal class Converter {
        ...
        private static async Task<DirectoryInfo> GenerateMainConfigFile(DirectoryInfo darActorFolder, DirectoryInfo oarModFolder,
            string oarConfigPath, string? modName, string? author = "") {
        ...
        private static async Task<DirectoryInfo> BuildOARDirectory(DirectoryInfo darActorFolder, DirectoryInfo oarModFolder,
            string oarConfigPath, bool overwrite, string? modName, string? modAuthor) {
            DirectoryInfo oarAnimationsFolder =
                await GenerateMainConfigFile(darActorFolder, oarModFolder, oarConfigPath, modName, modAuthor);
            DirectoryInfo darConditionsFolder =
                new DirectoryInfo(Path.Combine(darActorFolder.FullName, ANIMATION_FOLDER, DAR_FOLDER, DAR_CONDITIONS_FOLDER));
            DirectoryUtils.CopyDirectory(darConditionsFolder, oarAnimationsFolder, overwrite, true);

            return oarAnimationsFolder;
        }

        private static async Task<List<DirectoryInfo>> BuildOARDirectories(DirectoryInfo oarModFolder, string darModFolder, bool overwrite,
            string? modName, string? modAuthor) {
            ...
            foreach (DirectoryInfo darActorFolder in darActorFolders) {
                if (darActorFolder.GetDirectories(@"_1stperson\animations\DynamicAnimationReplacer\_CustomConditions").Length > 0) {
                    string oar1stPersonConfigPath = Path.Combine(ACTOR_FOLDER, darActorFolder.Name, FIRST_PERSON_FOLDER, ANIMATION_FOLDER,
                        OAR_FOLDER);
                    DirectoryInfo firstPersonDARDirectory = new DirectoryInfo(Path.Combine(darActorFolder.FullName, FIRST_PERSON_FOLDER));
                    DirectoryInfo oarAnimations1stPersonFolder = await BuildOARDirectory(firstPersonDARDirectory, oarModFolder,
                        oar1stPersonConfigPath, true, modName, modAuthor);
                ...
                DirectoryInfo oarAnimationsFolder =
                    await BuildOARDirectory(darActorFolder, oarModFolder, oarConfigPath, true, modName, modAuthor);
        ...
        public static async Task convertDARtoOAR(string darModFolder, string oarModFolderPath, string? modName, string? modAuthor) {
